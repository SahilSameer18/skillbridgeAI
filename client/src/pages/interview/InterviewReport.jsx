...
const NAV_ITEMS = [
    { id: 'technical', label: 'Technical', icon: (<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>) },
    ...
    { id: 'roadmap', label: 'Roadmap', icon: (<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="3 11 22 2 13 21 11 13 3 11"/></svg>) },
]
    ...
    return (
        <div className="animate-fade-in">
            ...
            <div className="flex gap-2 mb-6 lg:hidden overflow-x-auto pb-1">
                {NAV_ITEMS.map(item => (
                    <button key={item.id} onClick={() => setActiveNav(item.id)}
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all duration-200 ${activeNav === item.id ? 'text-white' : 'text-slate-500 bg-slate-900/50 border border-slate-800'}`}
                        style={activeNav === item.id ? {background:'linear-gradient(135deg,rgba(6,182,212,0.2),rgba(168,85,247,0.2))', border:'1px solid rgba(6,182,212,0.3)'} : {}}>
            ...
            <div className="flex gap-6">
                ...
                <aside className="hidden lg:flex flex-col gap-2 w-52 shrink-0">
                    ...
                    {NAV_ITEMS.map(item => (
                        <button key={item.id} onClick={() => setActiveNav(item.id)}
                            ...
                            style={activeNav === item.id ? {background:'linear-gradient(135deg,rgba(6,182,212,0.12),rgba(168,85,247,0.12))', border:'1px solid rgba(6,182,212,0.2)'} : {}}>
                ...
                {/* Right sidebar */}
                <aside className="hidden xl:flex flex-col gap-5 w-56 shrink-0">
                    {/* Score */}
                    <div className="rounded-2xl p-5 text-center" style={{background:'rgba(10,18,35,0.8)', border:'1px solid rgba(255,255,255,0.06)'}}>
                    ...
                    {/* Mobile Download */}
                    <button onClick={() => getResumePdf(interviewId)}
                        className="xl:hidden w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white"
                        style={{background:'linear-gradient(135deg,#06b6d4,#a855f7)'}}>
                        ✨ Download Resume
                    </button>
                </aside>
            ...
            <div className="xl:hidden mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="rounded-2xl p-5 flex items-center gap-4" style={{background:'rgba(10,18,35,0.8)', border:'1px solid rgba(255,255,255,0.06)'}}>
                    <div className="relative">
                        <svg width="70" height="70" className="-rotate-90">
                            <circle r={28} cx="35" cy="35" fill="none" stroke="rgba(51,65,85,0.6)" strokeWidth="5" />
                            <circle r={28} cx="35" cy="35" fill="none" stroke={scoreColor}
                                strokeWidth="5" strokeLinecap="round"
                                strokeDasharray={2 * Math.PI * 28}
                                strokeDashoffset={2 * Math.PI * 28 - (report.matchScore / 100) * 2 * Math.PI * 28}
                                style={{transition:'stroke-dashoffset 1.2s ease'}}
                            />
                        </svg>
                        <div className="absolute inset-0 flex items-center justify-center">
                            <span className="text-base font-bold text-white">{report.matchScore}%</span>
                        </div>
                    </div>
                    <div>
                ...
                <div className="rounded-2xl p-5" style={{background:'rgba(10,18,35,0.8)', border:'1px solid rgba(255,255,255,0.06)'}}>
                    <p className="text-xs text-slate-500 mb-3">Skill Gaps</p>
                    <div className="flex flex-wrap gap-1.5">
                        {report.skillGaps.slice(0, 5).map((gap, i) => {
                            ...
                            return <span key={i} className={`text-xs px-2 py-0.5 rounded-lg border ${styles[gap.severity?.toLowerCase()] || styles.low}`}>{gap.skill}</span>
                        })}
                        {report.skillGaps.length > 5 && <span className="text-xs text-slate-600">+{report.skillGaps.length - 5} more</span>}
                    </div>
                </div>
            </div>
        </div>
